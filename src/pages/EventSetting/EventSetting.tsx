...
import { Button, Divider, Image, Modal, Checkbox, Select, Tooltip, Typography, ColorPicker, Col, Row, Upload } from "antd";
...
const EventSetting = () => {
  ...
  const bgColorButton = useMemo<string>(
    () => (typeof colorButton === 'string' ? colorButton : colorButton!.toHexString()),
    [colorButton],
  );
  ...
  return (
    <div className={style["container"]}>
      ...
      <Modal title="CÀI ĐẶT HIỂN THỊ" open={isModalOpen} onOk={handleOk} 
      onCancel={handleCancel}
      footer={null}>
          <hr></hr>
          <div className={style["display__setting"]}>
            <Checkbox style={{ fontSize: '20px' }} onChange={onChange}>Hiển thị hình nền</Checkbox>
            <Checkbox style={{ fontSize: '20px' }} onChange={onChange}>Hiển thị logo công ty</Checkbox>
            <Checkbox style={{ fontSize: '20px' }} onChange={onChange}>Hiển thị tên sự kiên</Checkbox>
          </div>
          <div className={style["display__setting__button"]}>
          <Button type="primary" style={{ backgroundColor: "#c82333" }} onClick={handleCancel}>Đóng</Button>
          <Button type="primary" onClick={handleOk} style={{ backgroundColor: "rgb(21, 120, 21)", borderColor: "#28a745" }}>
            Xác nhận
          </Button>
        </div>
      </Modal>
      {/* type roll number modal */}
      <Modal title="CÀI ĐẶT NÂNG CAO" open={isModalOpenAdvanced} 
      ...
      footer={null}>
          ...
          <Typography.Text strong>
            SỐ LƯỢNG Ô:{" "}
            <Tooltip title="Số lượng ô quay số"></Tooltip>
          </Typography.Text>
          <Select
            value={slotCount}
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <Button type="primary" style={{ backgroundColor: "#c82333" }} onClick={handleCancelAdvanced}>Đóng</Button>
          <Button type="primary" onClick={handleOKAdvanced} style={{ backgroundColor: "rgb(21, 120, 21)", borderColor: "#28a745" }}>
            Lưu
          </Button>
        </div>
      </Modal>
    ...
    >
      ...
      <Row gutter={16} style={{ marginBottom: 16, marginTop: 10 }}>
        <Col span={16}>
          <Typography.Text>
            Ảnh nền (Kích thước khuyến nghị: 1920px X 1080px. Dung lượng {"≤"} 5Mb)
          </Typography.Text>
          <Upload
      ...
      <Row gutter={16} style={{ marginBottom: 16 }}>
        <Col span={16}>
          <Typography.Text>
            Logo công ty (Kích thước khuyến nghị: 250px X 250px. Dung lượng {"≤"} 5Mb)
          </Typography.Text>
          <Upload
            showUploadList={false}
            customRequest={({ file, onSuccess }) => {
      ...
      <div style={{ textAlign: "right", marginTop: 16 }}>
        <Button
        type="primary"
          style={{ backgroundColor: "#c82333", color: "#fff", marginRight: 8 }}
